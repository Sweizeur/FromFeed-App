...
class ShareViewController: UIViewController {
  let hostAppGroupIdentifier = "group.com.sweizeur.fromfeedapp"
  let shareProtocol = "com.fromfeed.app"
  let sharedKey = "com.fromfeed.appShareKey"
  ...
  private let accentBlue = UIColor(red: 0.04, green: 0.49, blue: 0.64, alpha: 1) // #0a7ea4
  ...
  private func setupSuccessModal() {
    ...
    // Conteneur type carte, centré, pas toute la largeur
    let card = UIView()
    ...
    // Icône succès (cercle bleu + checkmark)
  ...
  /// Pour les liens web : on enregistre dans l’App Group. Le modal succès reste affiché ;
  /// l’utilisateur ferme en tapant « Parfait ».
  private func dismissWithoutOpeningApp() {
    // Ne rien faire : le modal affiche déjà le succès, fermeture via le bouton.
  }

  private func redirectToHostApp(type: RedirectType) {
    // Pour les liens et le texte (souvent une URL), ne pas ouvrir l’app : fermer le modal après un délai.
